...
| `discover_datasets({keyword})` | candidates from The Graph's official Subgraph MCP (`search_subgraphs_by_keyword`, same Gateway key) with a paste-ready `mcp/config` entry each; the ones already sold are flagged |
...
```jsonc
{
  ...
  "pnl": { "endpoint": "https://api.studio.thegraph.com/query/1760032/open-book/v0.0.11" },
  "datasets": [
    {
